  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="bg-zinc-900 p-6 rounded-2xl shadow-md">
        ...
        <p className="text-3xl font-bold text-lime">{loading ? "Loading..." : summary.netProfit.toLocaleString()}</p>
      </div>
      <div className="bg-zinc-900 p-6 rounded-2xl shadow-md">
        <h2 className="text-xl font-semibold mb-2">🧾 EA Tax Paid</h2>
        <p className="text-3xl font-bold text-lime">{loading ? "Loading..." : summary.taxPaid.toLocaleString()}</p>
      </div>
    </div>
  )
}
